...
def main():
    ...
    cos_img = torch.dot(gc_img, gt_img).item() / max(gc_img.norm().item() * gt_img.norm().item(), 1e-12)
    ...
    print(f"  max abs diff={grad_at_image['cuda'].abs().max():.6e} / {grad_at_image['torch'].abs().max():.6e}")
